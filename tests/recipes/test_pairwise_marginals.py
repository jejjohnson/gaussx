...
class TestPairwiseMarginals:
    def test_output_shapes(self, getkey):
        ...
        transitions = jnp.tile(0.9 * jnp.eye(d), (T - 1, 1, 1))
        joint_means, joint_covs = pairwise_marginals(means, covs, transitions)
    ...
    def test_marginals_recover_individual(self, getkey):
        ...
        T, d = 4, 2
        means = jax.random.normal(getkey(), (T, d))
        A_mat = jax.random.normal(getkey(), (d, d)) * 0.5
        covs = jnp.tile(jnp.eye(d), (T, 1, 1))
        transitions = jnp.tile(A_mat, (T - 1, 1, 1))
        joint_means, joint_covs = pairwise_marginals(means, covs, transitions)
    ...
    def test_joint_covariance_symmetric(self, getkey):
        ...
        means = jax.random.normal(getkey(), (T, d))
        covs = jnp.tile(jnp.eye(d), (T, 1, 1))
        transitions = jnp.tile(0.8 * jnp.eye(d), (T - 1, 1, 1))
        _, joint_covs = pairwise_marginals(means, covs, transitions)
        for k in range(T - 1):
            assert jnp.allclose(joint_covs[k], joint_covs[k].T, atol=1e-8)

    def test_cross_covariance_correct(self, getkey):
        """Cross-covariance block equals P_k @ A_k.T."""
        T, d = 3, 2
        means = jax.random.normal(getkey(), (T, d))
        P = jax.random.normal(getkey(), (d, d))
        P = P @ P.T + jnp.eye(d)
        covs = jnp.tile(P, (T, 1, 1))
        A = jax.random.normal(getkey(), (d, d)) * 0.5
        transitions = jnp.tile(A, (T - 1, 1, 1))
        _, joint_covs = pairwise_marginals(means, covs, transitions)
        expected_cross = P @ A.T
        assert jnp.allclose(joint_covs[0, :d, d:], expected_cross, atol=1e-6)
    ...
    def test_joint_psd(self, getkey):
        ...
        P = P @ P.T + jnp.eye(d)
        covs = jnp.tile(P, (T, 1, 1))
        transitions = jnp.tile(0.5 * jnp.eye(d), (T - 1, 1, 1))
        _, joint_covs = pairwise_marginals(means, covs, transitions)
        for k in range(T - 1):

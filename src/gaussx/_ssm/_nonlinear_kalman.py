...
def _psd(matrix: Float[Array, "N N"]) -> lx.AbstractLinearOperator:
    """Wrap a dense covariance as a PSD-tagged operator."""
...
def _broadcast_noise(
    ...
    if dense.ndim == 2:
        return jnp.broadcast_to(dense, (T, *dense.shape))
    if dense.ndim == 3:
        return dense
    msg = f"{name} must have ndim 2 or 3, got {dense.ndim} (shape={dense.shape})."
...
def nonlinear_kalman_update(
    ...
    *,
    ...
    if joseph:
        ...
        # linear reduction.
        obs_eff = solve_rows(_psd(cov), cross_e.T, solver=solver)
        cov_upd = joseph_update(cov, gain, obs_eff, R_e)
    else:
...
def nonlinear_rts_step(
    ...
    *,
    ...
    # RTS gain.
    gain = solve_rows(_psd(cov_predicted), cross, solver=solver)  # (N, N)
...
def nonlinear_kalman_filter(
    ...
    *,
    ...
    Note:
        ...
        gain, whereas Joseph form is PSD for any $K$. The effective
        observation matrix it needs is the statistical-linearisation gain
        $H_{\text{eff}} = C^\top (P^-)^{-1}$ — the same quantity
        `gaussx.statistical_linear_regression` returns as ``A``, and
        exactly $H$ when ``obs_fn`` is linear. The two forms therefore
        coincide for affine maps and differ by $K \Omega K^\top$ otherwise,
        with $\Omega$ the statistical-linearisation residual.
    ...
    Args:
        ...
        integrator: Moment-matching rule. Defaults to
            `gaussx.UnscentedIntegrator`, which is derivative-free and
            exact for affine maps. Must supply a cross-covariance.
    ...
    Q_seq = _broadcast_noise(process_noise, T, "process_noise")
    R_seq = _broadcast_noise(obs_noise, T, "obs_noise")
...
def nonlinear_rts_smoother(
    ...
    *,
    ...
    Args:
        ...
        integrator: Moment-matching rule. Defaults to
            `gaussx.UnscentedIntegrator`; use the one the filter used.
